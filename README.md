...
13. 훅의 규칙
    ...
    - 페이지 244..
    - 두 번쨰 규칙은 리액트 함수형 리액트 ..

14. 시험 범위는 여기까지이고.
    - 개인 복습을 하자면~ 웹을 적용시키려면 반드시 index. js와 app.js 그리고 원래 보려면 jsx 파일을 수정을 해야만 함
    - 시험을 볼때 node 를 굳이 같이 압축시키지 않아야함. 이유로는 용량차지 (시험 볼때 이러면 감점)
    - react app 생성 
    - npx create-react-app 파일명
    - React는 JSX문법을 사용한다.
    - 항상 react를 사용할 때는 import 이름 from 경로를 사용해야만 함.
